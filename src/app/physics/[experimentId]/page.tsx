...
import ExperimentGuide from "@/components/physics/ExperimentGuide";
import ExperimentSummary from "@/components/physics/ExperimentSummary";
import ExperimentQuiz from "@/components/physics/ExperimentQuiz";
...
export default function PhysicsExperimentPage() {
  ...
  const loadExperiment = useCallback(async () => {
    ...
    setLoading(true); setError(null); setResult(null); setFollowUpReply(null); setAiGuided(true);
    try {
      const res = await fetch("/api/physics", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ experimentName: experiment.name, scientist: experiment.scientist }) });
      ...
      setStreaming(false);
      save("physics", experiment.name, fullText); setCurrentItem({ id: Date.now().toString(), subject: "physics", input: experiment.name, output: fullText, timestamp: Date.now() }); setShowSummary(true);
  ...
  async function handleFollowUp(question: string) {
    ...
    try {
      const res = await fetch("/api/physics", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ experimentName: experiment?.name || "", scientist: experiment?.scientist || "", question }) });
      if (!res.ok) { setFollowUpReply("😢 哎呀，出了点小问题，请再试一次吧～"); setFollowUpLoading(false); setFollowUpStreaming(false); return; }
    ...
    } catch { setFollowUpReply("😢 网络好像走丢了，请检查网络后重试～"); } finally { setFollowUpLoading(false); setFollowUpStreaming(false); }
  }
  ...
  return (
    <div className="relative min-h-[calc(100vh-3.5rem)]">
      <div className="max-w-3xl mx-auto px-6 md:px-12 py-12">
        <p className="text-[11px] tracking-[0.12em] text-[#B0A0C0] mb-8 font-medium">⚛️ 课程 / 物理 / {experiment.name}</p>
        <div className="mb-2"><h2 className="text-[40px] md:text-[48px] font-[900] tracking-[0.04em] leading-[1.1] text-[#3D3D4E]" style={{ fontFamily: "var(--font-cartoon), 'YouYuan', sans-serif" }}>{experiment.name}</h2><div className="flex items-center gap-2 mt-1"><p className="text-[15px] font-medium tracking-[0.02em] text-[#B0A0C0]">👨‍🔬 {experiment.scientist}</p>{experiment.difficulty && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: ACCENT, background: `${ACCENT}15` }}>{experiment.difficulty}</span>}</div></div>
        <div className="flex items-center gap-3 mb-6">
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => router.push("/physics")} className="flex items-center gap-1.5 px-4 py-2 rounded-2xl text-[12px] font-bold bg-white border-2 border-[#E8E0F0] text-[#6B6B7B] hover:border-[#D0C0F0] transition-all"><ArrowLeft className="w-3.5 h-3.5" /> 返回</motion.button>
        ...
        <div className="mt-6">
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={loadExperiment} disabled={loading}
            ...
            style={{ color: ACCENT, fontFamily: "var(--font-cartoon), 'YouYuan', sans-serif" }}><Sparkles className="w-5 h-5" />{loading ? "🤖 AI 老师正在分析实验..." : aiGuided ? "🔄 再问一次 AI 老师" : "🤖 遇到困难？点击我，我来教你做实验~"}</motion.button>
        </div>
        ...
        {/* 统一追问模块 */}
        <FollowUpQuestions subject="physics" accentColor={ACCENT} onAsk={handleFollowUp} loading={followUpLoading} reply={followUpReply} streaming={followUpStreaming} />
